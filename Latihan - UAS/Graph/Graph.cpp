...
bool isConnected(adrG G, infoG start, infoG end) {
	...
	while (current != nullptr) {
		...
		if (current->info.data = start.data) {
			while (child != nullptr) {
				if (child->dist.data == end.data) return true;
...
adrG findG(adrG G, infoG target) {
	if (G == nullptr) cout << "Graph Kosong!!" << endl;
		...
		adrG current = G;
		while (current != NULL) {
			if (current->info.data == target.data) return current;
			current = current->next;
		}
	}
}
...
void addEdge(adrG& G, infoG x_target, infoG y_target) {
	if (G == nullptr) cout << "Graph Kosong" << endl;
		...
		if (x != nullptr && y != nullptr) {
			adrGEdge dwEdge = allocEdgeG(y->info.data);
...
void printG(adrG& G) {
	if (G == nullptr) cout << "Graph Kosong" << endl;
		...
		while (current != nullptr) {
			adrGEdge child = current->edge;
			cout << current->info.data << " : ";
			while (child != NULL) {
				cout << child->dist.data << " ";
				child = child->edge_next;
			}
			cout << endl;
			current = current->next;
